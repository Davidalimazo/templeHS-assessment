...
const DoctorCard: FC<DoctorType> = ({avatar, acceptVirtualVisitOnly, name, title, initials, description, availability}) => {
    ...
    const isToday=(dateStr:string)=>{
       ...
       let today = todaydate[0] ==="Today";
        ...
        return today ? "Today" : `${dayName.slice(0, 3) + " "+ getMonth(monthIndex) + "/"+ dayIndex}`
    }

    //@ts-ignore
   const myArrow =({ type, onClick, isEdge })=> {
        //@ts-ignore
        const pointer = type === consts.PREV ? <PiCaretLeftLight className='w-[25px]' fontSize="25px"/> : <PiCaretRightLight className='w-[25px]' fontSize="25px"/>
        return (
  ...
  <div className="mt-5 text-[14px]">
    ...
    <p className="mt-5 font-semibold text-sm text-green-600">Check full profile and availability</p>
    </div>
</div>
}
